...
export class VestaConnection {
  ...
  fetch(channelId: string, fromSequence: number, options?: { toSequence?: number; limit?: number }): void {
    this.sendRaw({
      type: "FETCH",
      channelId,
      fromSequence,
  ...
  off<K extends EventKey>(event: K, listener: VestaConnectionEvents[K]): this {
  ...
  private emit<K extends EventKey>(event: K, ...args: Parameters<VestaConnectionEvents[K]>): void {
