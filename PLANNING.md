...
| 8   | **Port projection primitives to TS / Python** | `VestaCore.Projections.*` (`EventReducer`, `AppendOnlyLog`, `LwwRegister`, `LwwMap`) is C#-only. TS and Python clients still hand-roll their own projections (e.g. `chess-web` presence map). Mirror the API in `vesta-client-ts` and `vesta-client-py` so SDK conveniences are at parity across languages.                                                                                                                                                                                                                         | Pure convenience layer — wire protocol is unaffected. Refactor at least one example per language as smoke test. |
