...
| 12b | **Channel hard-delete pruner**                 | Background sweep that hard-deletes the events of soft-deleted channels after a configurable grace period (`Admin:ChannelDeletionGraceSeconds`), then removes the tombstone row so the channel id can be reused. Mirrors `AppQuotaPrunerService` (Postgres-only, opt-in via `ChannelDeletionPruner:Enabled`). When #10’s projection snapshots cover all reducers, deletion can also mean "snapshot final state + drop events".                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | Depends on #12a. Cheap once the soft-delete tombstone is in place.                                                                                                                                                                                                                  |
| 12c | **Admin HTTP API + GUI (RabbitMQ-style)**      | Read-mostly HTTP surface for ops: `GET /admin/channels`, `GET /admin/channels/{id}` (size, event count, members, deleted_at), `DELETE /admin/channels/{id}`, `GET /admin/apps`, `GET /admin/apps/{id}` (with quotas + storage rollup), `GET /admin/metrics` (active connections, events/sec). Authenticated via Ed25519 admin key (HTTP signature header, or signed bearer challenge — design TBD). On top of that, ship a small single-page GUI in the spirit of RabbitMQ Management: per-server overview, channels list with filtering, channel detail (recent events, members, quotas), apps list, force-delete buttons. Served by VestaServer itself under `/admin/` so a relay can be administered without deploying a separate service.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | Depends on #12a. The GUI is the operator-facing companion to #14 (observability) and complements #16 (client portal) by giving operators what end-users get for themselves.                                                                                                         |
