...
| 20  | **Server independence (fallback relays + owner-signed manifest)**           | ✅ **Done (Layers B + A + D core).** A Vesta app survives its designated relay/developer going away. **Layer B — failover:** `VestaConnection` now holds an ordered relay-candidate list (replacing the single server URI) and walks it on connect + reconnect; new `ConnectAsync(IReadOnlyList<Uri>, …)` (single-`Uri` overload kept), `ActiveRelay` / `Relays`, `OnRelaySwitched`, `UpdateRelayCandidates`, `SwitchRelayAsync`. **Layer A — owner-signed relay manifest:** `VestaCore.Relay.RelayManifest` (`{appId, version, issuedAt, relays[{url,priority,label}], escapeFallbacks[{url,validFrom}], ownerPublicKey, signature}`) + `ManifestSigner` (RFC 8785 JCS + Ed25519, signs all but `signature`). Published as a normal event (`type vesta.relay-manifest`) on channel `{appId}/vesta/relays`; clients auto-subscribe when a directory is attached, verify against the **compiled-in app-owner key**, and adopt only newer `version`s (anti-rollback). `VestaClient.Relay.RelayDirectory` ties config + per-user override + cached manifest into the resolved candidate order (override > manifest relays / active escape fallbacks > app defaults), with file + in-memory override/manifest stores. New `VestaAppConfig { AppId, OwnerPublicKey, DefaultRelays }`. **Layer D — reconfigure UI:** demonstrated in `examples/chess-web` only (relay panel: active relay, candidate switch, set/clear personal override via `LocalStorage*` stores, manifest-applied banner); `examples/ChatRoom.CLI` accepts a comma-separated relay list as a failover smoke test. Mirrored across all three SDKs (C# / TS `relay.ts` / Python `relay.py`) with byte-identical signing input. Tests: C# 28 (`RelayResolutionTests`, `RelayDirectoryTests`, `ManifestSignerTests`), TS 10 (`relay.test.mjs`), Python `test_relay.py` (parity; not run — no interpreter in dev env). **Zero relay/server changes** — the manifest is just signed events; the relay stays a pure relay. **Deferred:** Layer C (re-seed a fresh/empty relay from clients' local signed-event logs — until then the owner publishes the new manifest to BOTH old + new relay during migration); a localhost HTTP reconfigure surface for CLI/desktop apps without a web UI; server-side owner-only-write hardening on `{appId}/vesta/relays`; Atrium "adopt / rescue an abandoned app" (Layer E, proprietary). See [docs/protocol.md](docs/protocol.md#relay-manifests-server-independence). |
