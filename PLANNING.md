...
| 9b  | **Per-app quotas & rate limits**              | 🟡 **Partial.** Three quotas enforced today: `max_payload_bytes` (UTF-8 byte count of `payload + metadata` checked in PUBLISH), `publish_rate_per_minute` (in-process token bucket keyed by `(appId, clientId)`, see `AppRateLimiter`), and `max_channels` (counted from the `channels` table in CREATE_CHANNEL via `IChannelAccessStore.CountChannelsByAppAsync`). New error frames `ERROR { code: "QUOTA_EXCEEDED" / "RATE_LIMITED" }`. `IAppStore.SetQuotasAsync` exposed on both in-memory and Npgsql backends; `AppQuotas` record carries all six budget fields (three enforced, three reserved). Still deferred: `max_events_per_channel`, `retention_days`, `total_storage_bytes` — these need a background pruner + cached rollup, tracked as a separate slice. Multi-host rate limiting (shared bucket) tracked under #15.                 | See `src/VestaServer/Connections/AppRateLimiter.cs`, the `EnforcePublishQuotasAsync` / `EnforceMaxChannelsAsync` helpers in `ProtocolHandler.cs`, `AppQuotasTests`, and `AppRateLimiterTests`. |
