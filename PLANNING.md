...
| 9a  | **App registration & namespace enforcement**  | One Vesta server hosts many apps from many developers. Today any client can publish into any channel ID, and apps are only _encouraged_ (not required) to namespace as `{app}/...`. Add an `apps` table (`app_id` slug, `owner_client_id` Ed25519 pubkey, `created_at`, plus quota columns reserved for #9b) and a `REGISTER_APP` protocol message. When `Vesta:RequireAppRegistration=true`, validate that the first slug segment of every channel ID in PUBLISH / SUBSCRIBE / CREATE_CHANNEL matches a registered `app_id`; reject with `ERROR { code: "UNKNOWN_APP" }` otherwise. Default off in dev (open mode preserved), on in prod. Owner can grant additional admin keys for the app.                                                                                                                                                       | Small, mostly metadata. Wire protocol gains `REGISTER_APP` + `ERROR` frame; no change to existing messages. Existing channels grandfathered until cutover. |
| 9b  | **Per-app quotas & rate limits**              | Once apps are a first-class concept (#9a), attach budgets to the `apps` row so a single misbehaving or popular app can't fill the events table or hog fan-out. Limits: `max_channels`, `max_events_per_channel` (with background pruner), `max_payload_bytes` (PUBLISH validation), `publish_rate` per client (token bucket in connection handler), `retention_days` (background pruner), `total_storage_bytes` (hard ceiling checked from a cached rollup on append). New error frames: `QUOTA_EXCEEDED`, `RATE_LIMITED`. New hosted services: `AppPrunerService`, `QuotaAccountingService`. Server-only change; clients only need to learn the new error codes.                                                                                                                                                                                   | Larger — needs the pruner + accounting plumbing. Ship after #9a so quotas have a stable owner key to attach to.                                            |
...
| 11  | **Reconnect / outbox correctness audit**      | Audit and harden the publish path under flaky networks: client publishes → server stores+ACKs → ACK dropped → client reconnects and republishes. Dedup-by-event-id should cover it, but add explicit integration tests with simulated network kills mid-publish, mid-ACK, mid-batch. Verify the outbox doesn't lose events on hard process crash. Cover all three clients (`VestaClient.Tests/OfflineOutboxSyncTests` exists for C# — mirror it for TS/Python).                                                                                                                                                                                                                                                                                                                                                                                     | Correctness work — no new feature, but the SDK can't claim "offline-first" without this signed off.                                                        |
| 12  | **Channel deletion & admin tools**            | Today "channels live forever once created" (POC rule). Add `DELETE_CHANNEL` (admin only), soft-delete via tombstone row + hard-delete background sweep, and a small admin surface (CLI or HTTP) for ops: list channels, show channel size, force-delete, list apps. Depends on #9a for app ownership and is much more useful after #10 (snapshots) so deletion can also mean "snapshot + drop old events".                                                                                                                                                                                                                                                                                                                                                                                                                                          | Required before public deployment. Ties together #9a, #9b retention, and #10.                                                                              |
