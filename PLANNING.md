...
| 16  | **Client portal (end-user self-service)**      | ✅ **Done — implemented as the standalone [`vesta_atrium`](../../vesta_atrium) repo, NOT as a `/portal/` API inside VestaServer.** Design decision: the relay stays a **pure relay**; the portal is the proprietary managed-cloud control plane ("the GitHub to Vesta's git") that sits on top of the relay's existing `/admin/*` contract — VestaServer gained zero portal/account/billing code. Atrium is a Blazor Web App (Interactive Server, .NET 10) referencing `VestaCore`, with ASP.NET Core Identity (email/password) accounts over PostgreSQL. Flow: developer signs up → generates an Ed25519 app identity **client-side in the browser** (`@noble/ed25519` over JS interop — private key never reaches Atrium) → registers the app on the managed relay via WS `REGISTER_APP` (#9a) → downloads the identity file. Atrium's backend then **proves ownership** (reads the app back via `GET /admin/apps/{id}`, matches `ownerClientId`) and applies free-tier quotas via `PATCH /admin/apps/{id}/quotas`, authenticating with the operator admin key (challenge → sign → bearer, server-to-server — the admin key is never exposed to tenants). Per-app dashboard shows usage-vs-quota: storage from the admin rollup, message counts summed across channels (display/metering only until the relay grows a cumulative message-count quota on the PUBLISH hot path). `TierCatalog` is the single source mapping a tier to its concrete `RelayAppQuotas`; Free tier is live, Pro is modelled but **billing is out of scope** (upgrade is a placeholder). 8 provisioning unit tests pass (EF InMemory + fake relay). **Deferred → #16b:** key rotation, identity revocation, importing an existing identity, and key-loss recovery (the downloaded app key is download-once today).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | New TODO. Pairs with #12c (operator GUI) and #9a (apps + ownership). Decide auth model first, then split into concrete sub-slices.                                                                                                                                                    |
| 16b | **Client portal — key lifecycle**              | Follow-up to #16 (Atrium). Not yet built: **rotate** an app's Ed25519 key, **revoke** a compromised identity, **import** an existing identity instead of generating one, and a **recovery** story for a lost app key (currently download-once, no recovery). Several of these need relay-side support that doesn't exist yet — e.g. an `/admin/*` endpoint to rebind an app's `owner_client_id`, and/or multiple owner keys per app — so this is a joint Atrium + VestaServer `/admin/*` design pass. Billing / paid tiers (Stripe) also remain out of scope until this lands. |
