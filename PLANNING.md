...
| 16  | **Client portal (end-user self-service)**      | ✅ **Done — implemented as the standalone [`vesta_atrium`](../../vesta_atrium) repo, NOT as a `/portal/` API inside VestaServer.** Design decision: the relay stays a **pure relay**; the portal is the proprietary managed-cloud control plane ("the GitHub to Vesta's git") that sits on top of the relay's existing `/admin/*` contract — VestaServer gained zero portal/account/billing code. Atrium is a Blazor Web App (Interactive Server, .NET 10) referencing `VestaCore`, with ASP.NET Core Identity (email/password) accounts over PostgreSQL. Flow: developer signs up → generates an Ed25519 app identity **client-side in the browser** (`@noble/ed25519` over JS interop — private key never reaches Atrium) → registers the app on the managed relay via WS `REGISTER_APP` (#9a) → downloads the identity file. Atrium's backend then **proves ownership** (reads the app back via `GET /admin/apps/{id}`, matches `ownerClientId`) and applies free-tier quotas via `PATCH /admin/apps/{id}/quotas`, authenticating with the operator admin key (challenge → sign → bearer, server-to-server — the admin key is never exposed to tenants). Per-app dashboard shows usage-vs-quota via the relay's `GET /admin/apps/{id}/usage` rollup (single call, no more N+1 per-channel summing) — both storage **and message count** are now relay-enforced (`max_messages_per_month`, see #9b). `TierCatalog` is the single source mapping a tier to its concrete `RelayAppQuotas`; Free and **Pro are both live** with Stripe Checkout + Customer Portal (**test mode only** — see #16b). 11 provisioning unit tests pass (EF InMemory + fake relay). **Deferred → #16b:** done — see below.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | New TODO. Pairs with #12c (operator GUI) and #9a (apps + ownership). Decide auth model first, then split into concrete sub-slices.                                                                                                                                                    |
| 16b | **Client portal — key lifecycle & billing**    | ✅ **Done (scoped).** **Billing:** account-level Stripe subscription (one customer/subscription per Atrium account, cascading to every owned app) — `BillingService` drives Checkout Sessions (subscription mode) and the Customer Portal directly from Blazor Interactive Server components (no separate authenticated HTTP API needed); `POST /billing/webhook` verifies the Stripe signature (`EventUtility.ConstructEvent`) before trusting anything, is idempotent via a `ProcessedWebhookEvents` table, and on `checkout.session.completed` / `customer.subscription.*` re-applies `TierCatalog.For(tier).Quotas` to every `DeveloperApp` the account owns. Test-mode keys only (`sk_test_`/`whsec_`), wired through Key Vault + Bicep; billing is out of scope for live payments. **Key lifecycle:** deliberately simplified from "multiple owner keys" to an **operator-assisted single-owner rebind** — new `PATCH /admin/apps/{id}/owner` on the relay (`IAppStore.SetOwnerAsync`, no schema change needed since `owner_client_id` already existed) lets Atrium rebind an app's recognized owner to a new client id. Atrium's `AppProvisioningService.RotateKeyAsync` generates a new browser identity, downloads it, and rebinds; `NewApp.razor` gained an "import an existing identity" toggle (paste a public key instead of generating one). Standalone "revoke" was **not** built as a separate action — rotating to a new key already revokes the old one implicitly, since the relay has no other enforcement point keyed on `owner_client_id` today. Recovery is this same rebind, documented as a managed-cloud-only convenience (self-hosters keep the pure self-sovereign, no-recovery model). |
...
| 18  | **Infrastructure-as-Code (Bicep)**             | ✅ **Done.** `.github/infra/` Bicep stack provisions the relay's Azure footprint subscription-scoped (`main.bicep` + `modules/`): resource group, Burstable PostgreSQL Flexible Server (`Standard_B1ms` for dev), Log Analytics + Application Insights, Key Vault (DB connection string), and the relay Web App (Linux, .NET 10, WebSockets + Always On, system-assigned identity granted **Key Vault Secrets User**). App settings wire `ConnectionStrings__Vesta` via `@Microsoft.KeyVault(...)` references and seed `Admin__BootstrapPublicKeys__0` with the Atrium operator public key. Dev params in `.github/infra/params/dev.bicepparam` read secrets from env vars (never committed). Cost controls: `.github/infra/scripts/dev-stop.ps1` / `dev-start.ps1` (stop/start web app + Postgres) and `dev-teardown.ps1` (`az group delete`). `main_vestaserver.yml` gained an opt-in `infrastructure` job (`workflow_dispatch`). Stack is **Atrium-agnostic** — the managed portal has its own separate Bicep in the `vesta_atrium` repo. Postgres pinned to v16 (Azure Flexible Server max). Not yet validated live (no Azure in dev env). See the [Deployment & Operations](#deployment--operations-relay-stack) runbook below. |
| 19  | **Let Atrium build standalone (decouple from the `VestaCore` source ref)** | ✅ **Done — chose Option B.** Atrium reimplements the ~40 lines of primitives it used (`src/Atrium.Web/Vesta/VestaIdentity.cs`, `Base64Url.cs`, `AppId.cs` under namespace `Atrium.Web.Vesta` — Ed25519 via **NSec.Cryptography** 26.4.0, `DeriveClientId` = `base64url(sha256(pubkey))[:22]`, Base64Url via the BCL `System.Buffers.Text.Base64Url`, `AppId.IsValid` regex). The `VestaCore` `ProjectReference` is removed, Atrium builds standalone, and all 8 provisioning tests pass. Vesta stays a clean, versatile SDK with **zero packaging burden** (no NuGet publishing); interop is enforced at runtime (the relay verifies the signature + app id, so any drift fails loudly). _Original analysis retained below._ <br><br>Today the proprietary Atrium repo consumed `VestaCore` via a relative `ProjectReference` (`..\..\..\Vesta\src\VestaCore\VestaCore.csproj`), so it only builds with the Vesta repo checked out as a sibling. Atrium's **entire** use of VestaCore is four non-HTTP primitives: `VestaIdentity.FromPrivateKey/.Sign/.PublicKey` (sign the admin-challenge nonce), `VestaIdentity.DeriveClientId` (ownership proof), `Base64Url.Encode/Decode`, and `AppId.IsValid`. Three ways to decouple: **(A) Publish `VestaCore` as a NuGet package** — add metadata to `src/VestaCore/VestaCore.csproj` (`PackageId` `Vesta.Core`, authors, license, `RepositoryUrl`, deterministic/`ContinuousIntegrationBuild`, `GenerateDocumentationFile`; keep `InternalsVisibleTo`), version from a Git tag via `MinVer`/`Nerdbank.GitVersioning`, new `publish-vestacore.yml` workflow (`dotnet pack` + `nuget push` to **nuget.org** on tag `vestacore-v*`, `NUGET_API_KEY` secret), then Atrium switches to `PackageReference Vesta.Core`. Only `VestaCore` ships — `VestaServer`/`VestaClient` stay unpublished. **(B) Atrium reimplements the ~40 lines and drops the dependency entirely** — Ed25519 via **NSec.Cryptography** directly (what `VestaIdentity` already wraps), `DeriveClientId` = `base64url(sha256(pubkey))[:22]`, Base64Url via the **BCL `System.Buffers.Text.Base64Url`** (net9+), `AppId` regex. Zero packaging work; relay stays a pure relay; interop enforced at runtime (relay verifies the signature + app id, so drift fails loudly). **(C) NSwag is _not_ a substitute** — it generates an HTTP client/DTOs from an OpenAPI spec, so it could only replace the hand-mirrored `Relay/RelayModels.cs` shapes, **not** the crypto coupling; and the relay has no OpenAPI doc today, so using NSwag would mean _adding_ Swashbuckle/OpenAPI surface to VestaServer (less clean, not more). **Decision driver:** if Atrium's use of Vesta primitives will grow (event signing, protocol messages, projections) → **A** (one canonical SDK). If it stays "sign a nonce + validate a slug" → **B** (cleanest; Vesta needs no packaging at all). |
| 20  | **Server independence (fallback relays + owner-signed manifest)**           | ✅ **Done (Layers B + A + D core; default-on in C#).** A Vesta app survives its designated relay/developer going away. **Default-on (C#):** relay independence is no longer opt-in plumbing — `VestaConnection` now **requires** a `VestaAppConfig` (the trust anchor + default relays) and auto-builds a file-backed `RelayDirectory` (`RelayDirectory.CreateDefault`, cached under `~/.vesta/relays/{appId}.{override,manifest}.json`), so manifest discovery + adoption + multi-relay failover happen with no per-app wiring; `ConnectAsync(IReadOnlyList<string> channels, …)` resolves relays from the config, and `SetUserRelayOverrideAsync` / `ClearUserRelayOverrideAsync` expose the local escape hatch. _(Breaking: the old `VestaConnection(clientId, localStore?, identity?)` ctor is replaced by `VestaConnection(clientId, appConfig, localStore?, identity?, relayDirectory?)`.)_ **Layer B — failover:** `VestaConnection` holds an ordered relay-candidate list (replacing the single server URI) and walks it on connect + reconnect; `ConnectAsync(IReadOnlyList<Uri>, …)` (single-`Uri` overload kept), `ActiveRelay` / `Relays`, `OnRelaySwitched`, `UpdateRelayCandidates`, `SwitchRelayAsync`. **Layer A — owner-signed relay manifest:** `VestaCore.Relay.RelayManifest` (`{appId, version, issuedAt, relays[{url,priority,label}], escapeFallbacks[{url,validFrom}], ownerPublicKey, signature}`) + `ManifestSigner` (RFC 8785 JCS + Ed25519, signs all but `signature`). Published as a normal event (`type vesta.relay-manifest`) on channel `{appId}/vesta/relays`; clients auto-subscribe (now always, since a directory is always attached in C#), verify against the **compiled-in app-owner key**, and adopt only newer `version`s (anti-rollback). `VestaClient.Relay.RelayDirectory` ties config + per-user override + cached manifest into the resolved candidate order (override > manifest relays / active escape fallbacks > app defaults), with file + in-memory override/manifest stores. New `VestaAppConfig { AppId, OwnerPublicKey, DefaultRelays }`. **Layer D — reconfigure UI:** demonstrated in `examples/chess-web` (relay panel: active relay, candidate switch, set/clear personal override via `LocalStorage*` stores, manifest-applied banner); `examples/ChatRoom.CLI` now ships `/relays`, `/relay use|clear`, and `/publish-manifest` commands (self-signing demo: owner key defaults to the client's own key, overridable via `VESTA_APP_OWNER_KEY`) over a comma-separated default-relay list. Mirrored across all three SDKs (C# / TS `relay.ts` / Python `relay.py`) with byte-identical signing input. **Default-on currently C# only — TS/Python still attach the directory explicitly (port pending confirmation).** Tests: C# 29 (`RelayResolutionTests`, `RelayDirectoryTests` incl. `CreateDefault`, `ManifestSignerTests`), TS 10 (`relay.test.mjs`), Python `test_relay.py` (parity; not run — no interpreter in dev env). **Zero relay/server changes** — the manifest is just signed events; the relay stays a pure relay. **Deferred:** Layer C (re-seed a fresh/empty relay from clients' local signed-event logs — until then the owner publishes the new manifest to BOTH old + new relay during migration); a localhost HTTP reconfigure surface for CLI/desktop apps without a web UI; server-side owner-only-write hardening on `{appId}/vesta/relays`; Atrium "adopt / rescue an abandoned app" (Layer E, proprietary). See [docs/protocol.md](docs/protocol.md#relay-manifests-server-independence). |
| 21  | **Server-to-server discovery (federation)**                                | ✅ **Done (server + protocol + C# client + demo; TS/Py deferred).** Extends #20 from *planned* migration (owner pre-lists relays in a manifest) to *recovery*: when a client's relays are all failing and no fresh manifest exists, it can discover **other** relays hosting the same app — **without a central hub**. **Design (all confirmed):** (1) **gossip mesh, anti-entropy pull** (Matrix-style), not a DHT; (2) **dual opt-in** — the operator sets `Discovery:Enabled` *and* the app owner sets a per-app `discoverable` flag; crucially the flag is **relay-side metadata** (new `apps.discoverable` column, `REGISTER_APP { discoverable }` / `PATCH /admin/apps/{id}/discoverable`), **not** parsed from any event payload, so architecture rule #2 ("relay never interprets payloads") holds; (3) **HTTP `/federation/*` pull transport**, no new WS message; (4) **discovered relays are show-only** — owner-signed manifest relays stay the sole *automatic* failover tier, the user adopts a discovered relay manually via the existing `SetUserRelayOverrideAsync` escape hatch. **Core:** `VestaCore.Relay.ServerDescriptor` (`{relayPublicKey, urls[], apps[{appId, ownerClientId}], issuedAt, ttlSeconds, signature}`) + `DescriptorSigner` (RFC 8785 JCS + Ed25519, **self-signed** — the relay declares & signs with its own key). The descriptor advertises each app's `ownerClientId` (= `sha256(ownerPublicKey)[:22]`, what the app store already holds) so a client can cross-check without a second migration. **Server:** `Discovery` options (`Enabled`, `Seeds`, `PublicUrls`, `SigningKey`, `GossipIntervalSeconds`=60, `DescriptorTtlSeconds`=300, `MaxPeers`=256); `IServerDirectory`/`InMemoryServerDirectory` (last-write-wins by `issuedAt`, TTL eviction, peer cap); `LocalDescriptorProvider` (builds + signs from discoverable apps); relay identity from `SigningKey` or `{ContentRoot}/.vesta/relay-key.json`; `FederationEndpoints` (`GET /federation/{descriptor,peers,apps/{appId}}`, all unauthenticated reads of signed descriptors); `FederationGossipService` (background pull from seeds + known peers, verify + merge). All gated on `Discovery:Enabled`; EF migration `AddAppDiscoverable`. **Client (C#):** `VestaClient.Federation.FederationClient.DiscoverRelaysForAppAsync` / `ListAllRelaysAsync` — GET a reachable relay's federation surface, verify every descriptor signature, **drop relays whose advertised `ownerClientId` ≠ `VestaIdentity.DeriveClientId(VestaAppConfig.OwnerPublicKey)`** (spoof defence), return survivors as `DiscoveredRelay` (show-only, never auto-adopted). **Demo:** ChatRoom.CLI `/discover` (relays hosting this app) + `/discover all` (browse the mesh), adopt with the existing `/relay use <ws-url>`. **Deferred:** the TS/Python clients get no `FederationClient` yet (shelved per the client-parity note) — server + protocol shapes are the source of truth for the eventual port. |

---

## Deployment & Operations (relay stack)

Operational runbook for the relay's Azure footprint. The full design rationale is in TODO #18;
this section is the **how-to-deploy** companion — keep it current whenever `.github/infra/`,
the deploy workflow, or the required secrets change.

### What the stack provisions

All subscription-scoped from `.github/infra/main.bicep` (resource group `vesta-dev-rg`):
Burstable PostgreSQL Flexible Server (`vesta-dev-pg`, `Standard_B1ms`), Log Analytics +
Application Insights (`vesta-dev-logs` / `vesta-dev-ai`), Key Vault (`vesta-dev-kv`), and the
relay Web App (`vestaserver`, Linux `DOTNETCORE|10.0`, WebSockets + Always On, system-assigned
identity granted **Key Vault Secrets User**).

### One-time bootstrap (before the first deploy)

1. **Generate the relay operator keypair** (shared with Atrium):
   `dotnet run .github/infra/scripts/new-relay-keypair.cs`. It prints the **public** half
   (Vesta) and the **private** half (Atrium). Store the private half only in Atrium's secret
   store — Vesta only ever needs the public half.
2. **Set the deploy secrets** (GitHub Actions secrets + local env vars for manual runs):
   - `VESTA_PG_PASSWORD` — Postgres admin password.
   - `VESTA_RELAY_ADMIN_PUBLICKEY` — the operator **public** key from step 1 (base64url),
     seeded into `Admin__BootstrapPublicKeys__0`.
   - OIDC login reuses the existing `AZUREAPPSERVICE_*` secrets.

### Deploy

- **Manual:** `az deployment sub create --location norwayeast --template-file
  .github/infra/main.bicep --parameters .github/infra/params/dev.bicepparam` (with the two
  `VESTA_*` env vars set).
- **CI:** run `main_vestaserver.yml` with the opt-in `infrastructure` job (`workflow_dispatch`).
- **Deploy the relay first.** Its outputs `relayBaseUrl` / `relayWebSocketUrl` feed Atrium's
  `params/dev.bicepparam` (the portal stack can't be finalized until these exist).

### Verify (when Azure is available)

1. **Preview before applying:** `az deployment sub what-if --location norwayeast --template-file
   .github/infra/main.bicep --parameters .github/infra/params/dev.bicepparam`.
2. Relay `/health` returns **200** and the WebSocket endpoint accepts a HELLO.
3. The Web App's managed identity can read `vesta-dev-kv` (the `ConnectionStrings__Vesta`
   `@Microsoft.KeyVault(...)` reference resolves — no `KeyVaultReferenceFailure` in app settings).
4. Application Insights is receiving request/dependency telemetry.
5. `dev-stop.ps1` / `dev-start.ps1` cycle the web app + Postgres cleanly; `dev-teardown.ps1`
   removes the resource group.

### Cost controls

`.github/infra/scripts/dev-stop.ps1` (stop web app + Postgres when idle) /
`dev-start.ps1` (resume) / `dev-teardown.ps1` (`az group delete -n vesta-dev-rg`). The App
Service Plan keeps billing while it exists, so a full cost-stop is teardown. Key Vault stays
soft-deleted for 7 days after teardown — purge it to reuse the name.

### Gotchas

- **`vestaserver` is a globally-unique App Service name** — change it if taken.
- **Azure PostgreSQL Flexible Server tops out at major 16** (`postgresVersion` defaults `'16'`),
  even though the protocol docs aspire to PG18.
- The relay depends on **PostgreSQL `LISTEN`/`NOTIFY`** for fan-out — do not swap the store for
  Azure SQL.
- Not yet validated against live Azure (no subscription in the dev environment).
