...
export class BoardView {
    ...
    get statusText(): string {
        ...
        if (this.chess.isCheck())
            return `Check (${this.turn === "w" ? "White" : "Black"} to move)`;
    ...
    private render(): void {
        ...
        const ranks =
            this.opts.orientation === "w"
                ? [8, 7, 6, 5, 4, 3, 2, 1]
                : [1, 2, 3, 4, 5, 6, 7, 8];
        const files =
            this.opts.orientation === "w" ? FILES : [...FILES].reverse();

        const targets = new Set<Square>();
        if (this.selected) {
            for (const m of this.chess.moves({
        ...
        for (const rank of ranks) {
            for (const file of files) {
                ...
                if (
                    this.lastMove &&
                    (this.lastMove.from === square ||
                        this.lastMove.to === square)
                ) {
